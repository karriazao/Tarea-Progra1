...
{
    if (alturaTablero * anchoTablero > MAX_CELDAS) {
        int totalCeldas = MAX_CELDAS;
        this->alturaTablero = totalCeldas / MAX_ANCHO;
        this->anchoTablero = MAX_ANCHO;
        if (this->alturaTablero > MAX_ALTURA) {
            this->alturaTablero = MAX_ALTURA;
        }
    } else {
        this->alturaTablero = (alturaTablero > MAX_ALTURA || alturaTablero <= 0) ? MAX_ALTURA : alturaTablero;
        this->anchoTablero = (anchoTablero > MAX_ANCHO || anchoTablero <= 0) ? MAX_ANCHO : anchoTablero;
    }
...
bool Tablero::setAlturaTablero(int alturaTablero){
    if (alturaTablero > MAX_ALTURA || alturaTablero <= 0){
        return false;
    }
...
bool Tablero::setAnchoTablero(int anchoTablero)
{
    if (anchoTablero > MAX_ANCHO || anchoTablero <= 0){
        return false;
    }
